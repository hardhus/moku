fn main() {
    ...
    // own tests/examples (moku-volume-mount's own real-mount test already
    ...
    // when a volume is actually mounted, via WinFsp's own registry-based
    // lookup at that point (see winfsp::init::load_system_winfsp).
    // See moku-volume-mount/build.rs for why this checks the compile
